...
def reslog(line):
    ...
    with open(r'/home/mabbasib/MLN/logs/accSynth2.csv', 'a', newline='') as f:
        writer = csv.writer(f)
...
if __name__ == '__main__':
    ...
    name = '/scratch/mabbasib/FCLOGSI/Synth2' + (''.join(sys.argv[1:]))
    ...
    DT_tr = MakeHIratio(aI, transform=transform, Len=args.Len,val=1,val2=0)
    DT_va = MakeHIratio(aI, transform=transform, Len=args.Len,val=1,val2=1)
    sampler=MySamplerV(len(DT_va)//9)
    ...
    val_loader = torch.utils.data.DataLoader(
        DT_va,
        batch_size=batchsize,
        num_workers=6, pin_memory=True,sampler=sampler)
