...
parser.add_argument("--BS", type=int, default=32)  # 3 modeR==1 9 if modeR==0
...
parser.add_argument("--numepc", type=int, default=50)
...
def reslog(line):
    ...
    with open(r'/home/mabbasib/MLN/logs/ACC_F_RC3_9.csv', 'a', newline='') as f:
        writer = csv.writer(f)
...
if __name__ == '__main__':
    ...
    name = '/scratch/mabbasib/FCLOGS_F_RC3_9/finetune2' + (''.join(sys.argv[1:]))
    #name = '.\\logs\\finetune' + (''.join(sys.argv[1:]))
    logF = name + '.csv'
    netS = name + '.pth'
    if args.cont:
    
        netS2 = name = '/scratch/mabbasib/FCLOGSIA3f/Synth--Ra0' + (''.join(sys.argv[7:])) +'.pth'
    ...
    a, b, c = datPP3_r(root_dir='./Real/',Ra=args.Ra)
