...
def reslog(line):
    ...
    with open(r'/home/mabbasib/MLN/logs/accRealSynth.csv', 'a', newline='') as f:
        writer = csv.writer(f)
...
name = '/scratch/mabbasib/FCLOGSRI/RealSynth' + (''.join(sys.argv[1:]))
...
if __name__ == '__main__':
    ...
    DT_trS = MakeHIa(aI, transform=transform, Len=args.Len)
    # DT_va = MakeHI(c, transform=transformV, Len=args.Len)
    DT_trR = MakeH_ra(bR, transform=transform, val=0,Len=args.Len)
    DT_va = MakeH_ra(cR, transform=transformV, val=1,Len=args.Len)
