...
def reslog(line):
    ...
    with open(r'/home/mabbasib/MLN/logs/accRealSynth3.csv', 'a', newline='') as f:
        writer = csv.writer(f)
...
if __name__ == '__main__':
    ...
    name = '/scratch/mabbasib/FCLOGSRI3/RealSynth1' + (''.join(sys.argv[1:]))
    ...
    aR, bR, cR = datPP3_r(root_dir='/scratch/mabbasib/MLD/',Ra=args.Ra)
    aI, bS, cS = datPPI32(root_dir='./all_emo/')
    # DT_trS = MakeHI(aI, transform=transform, Len=args.Len)
    DT_trS = MakeHIratio2(aI, transform=transform, Len=args.Len,ratio=args.ratio,loopD=args.loopD)
    DT_trR = MakeH_r(bR, transform=transform, val=0,Len=args.Len)
    DT_va = MakeH_r(cR, transform=transformV, val=1,Len=args.Len)
    #print(len(DT_trR))
    #print(len(DT_va))
    #print(len(DT_trS))
    #aI, bS, cS = datPPI3(root_dir='./all_emo/')
    #DT_trS = MakeHI(aI, transform=transform, Len=args.Len)
    # DT_va = MakeHI(c, transform=transformV, Len=args.Len)
    
    DT_tr =torch.utils.data.ConcatDataset([DT_trR,DT_trS])
    ...
    reslog([args.ratio,args.BS,args.JobId,args.NN,args.hids,args.loopD,args.mode,args.Frz,args.cont,args.reduction,args.reduction3,args.Ra,args.Len,args.numepc,args.patience,args.Sc,args.wd,args.lr,111,preM, recM, fsM,J_s,best_acc])